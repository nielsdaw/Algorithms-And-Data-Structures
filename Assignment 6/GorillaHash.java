public class GorillaHash {
	...
	private static int T = 0;			// Number of k grams
	...
	public static void main(String[] args) {
		...
		while(StdIn.hasNextLine()){
			...
			if (nextSpecies.substring(0,1).equals(">")) {	// new species has been found
				String[] tmpArr = nextSpecies.split(" ",1); // remove the ">"
		...
		for (int i = 0; i < 12; i++ ) {		// compare all species
			int[] tmp = allSpecies[i].getVector();		// species 1 to compare

			for (int x = 0; x < 12; x++ ) {
				StdOut.println("species1: " + allSpecies[i].getName());
				int[] tmp2 = allSpecies[x].getVector();		// species 2 to compare
				StdOut.println("species 2: " + allSpecies[x].getName());
				StdOut.println(cos_angle(tmp, tmp2));
			}
		}
	}
